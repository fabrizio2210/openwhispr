...
test(
  "connects to an abstract session bus through Node's socket stream",
  { skip: process.platform !== "linux" },
  async (t) => {
    const abstractName = `/tmp/openwhispr-dbus-test-${process.pid}-${Date.now()}`;
    const abstractSocketPath = `\0${abstractName}`;
    const guid = "0123456789abcdef0123456789abcdef";
    ...
    const server = net.createServer((socket) => {
      ...
      socket.on("data", (data) => {
        if (!authenticated && data.includes(Buffer.from("\r\n"))) {
          authenticated = true;
          socket.write(`OK ${guid}\r\n`);
        }
      });
    });
    await new Promise((resolve, reject) => {
      server.once("error", reject);
      server.listen(abstractSocketPath, resolve);
    });

    const monitor = new LinuxOutlookNotificationMonitor({
      platform: "linux",
      desktop: "KDE",
      busAddress: `unix:abstract=${abstractName}`,
    });
    t.after(async () => {
      monitor.stop();
    ...
    monitor.start();
    assert.ok(monitor.connection, "the real D-Bus dependency should accept the Node socket stream");
    await once(monitor.connection, "connect", { signal: AbortSignal.timeout(2000) });

    assert.equal(monitor.connection.stream.remoteAddress, undefined);
