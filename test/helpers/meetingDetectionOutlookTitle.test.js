...
function createHarness({
  ...
  const engine = new MeetingDetectionEngine(
    ...
    windowManager,
    databaseManager,
    { meetingTitleContextProvider: contextProvider }
  );
  engine.broadcastToWindows = () => {};
  if (preferencesSynchronized) {
...
test("fails closed until notification preferences are synchronized", () => {
  const harness = createHarness({
    context: OUTLOOK_CONTEXT,
    preferencesSynchronized: false,
  });

  harness.engine.start();

  assert.equal(harness.audioDetector.startCount, 0);
  assert.equal(harness.contextProvider.startCount, 0);
  ...
  syncMeetingNotificationPreferences(
    harness.engine.windowManager.notificationPrefs,
    completeNotificationPreferences(),
    harness.engine
  );
  assert.equal(harness.audioDetector.startCount, 1);
  assert.equal(harness.contextProvider.startCount, 1);
  assert.equal(harness.engine._notificationsEnabledFor(NOTIFICATION_SOURCES.AUDIO), true);
  assert.equal(harness.prompts.length, 1);
  assert.equal(harness.prompts[0].detectionId, "calendar:calendar-before-sync");
});

test("drops a queued calendar reminder when synchronized preferences disable it", () => {
  const harness = createHarness({ preferencesSynchronized: false });
  harness.engine.start();
  harness.engine.handleCalendarReminder({
    id: "disabled-calendar-before-sync",
    calendar_id: "primary",
    summary: "Disabled calendar meeting",
    start_time: new Date().toISOString(),
    end_time: new Date(Date.now() + 60_000).toISOString(),
  });
  ...
  syncMeetingNotificationPreferences(
    harness.engine.windowManager.notificationPrefs,
    completeNotificationPreferences({ notifyCalendarReminders: false }),
    harness.engine
  );

  assert.equal(harness.prompts.length, 0);
  assert.equal(harness.engine._preSyncCalendarDetections.size, 0);
  assert.equal(harness.engine.activeDetections.size, 0);
});
...
test("applies disabled persisted notification preferences before monitoring", () => {
  const harness = createHarness({ context: OUTLOOK_CONTEXT, preferencesSynchronized: false });
...
test("does not monitor until the required typed meeting preferences are synchronized", () => {
  ...
  for (const invalidPreferences of [
    ...
    },
    {
      notificationsEnabled: true,
      notifyMeetingDetection: true,
      notifyUpdates: "invalid",
    },
    {
      notificationsEnabled: true,
      notifyMeetingDetection: true,
      meetingDetectionNotifications: "invalid",
    },
    {
      notificationsEnabled: true,
      notifyMeetingDetection: true,
    ...
    assert.equal(harness.contextProvider.startCount, 0);
    assert.equal(harness.engine.windowManager.notificationPreferencesSynchronized, false);
    assert.equal(harness.engine._notificationsEnabledFor(NOTIFICATION_SOURCES.CALENDAR), false);
    assert.equal(harness.engine._notificationsEnabledFor(NOTIFICATION_SOURCES.UPDATES), false);
  }
