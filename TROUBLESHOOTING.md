...
On Linux, OpenWhispr checks whether PulseAudio source outputs are uncorked. If
`pactl` is unavailable, it reads `pw-dump` and treats only `running` PipeWire
microphone input streams as active. Dormant, corked, `idle`, or `suspended`
streams left behind by Zoom, Plasma, or another application do not trigger a
meeting prompt. Dismissing a real prompt also suppresses repeats from the same
continuous stream until the microphone has been verifiably inactive for 60
seconds; a failed state check does not count as inactivity.
...
note, that reminder is consumed so it cannot name a later meeting. Google
