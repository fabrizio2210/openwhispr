...
- Title precedence is Calendar → Outlook notification → `New note`; the Outlook title also applies to manual meeting starts when a matching reminder remains cached
...
- Outlook-aware on KDE: if no connected calendar event matches, an eligible Edge/Outlook reminder supplies the title without linking a calendar event
...
- CI workflow: `.github/workflows/build-windows-mic-listener.yml` builds and publishes the pinned standalone helper from `resources/windows-mic-listener.c`
