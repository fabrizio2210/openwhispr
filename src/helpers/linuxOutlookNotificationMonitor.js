...
function getAbstractSocketPath(busAddress) {
  const abstractParameter = busAddress
    .slice(busAddress.indexOf(":") + 1)
    .split(",")
    .find((parameter) => parameter.startsWith("abstract="));
  return abstractParameter ? `\0${abstractParameter.slice("abstract=".length)}` : null;
}
...
class LinuxOutlookNotificationMonitor {
  ...
  _connect(addressIndex = 0) {
    ...
    try {
      ...
      const abstractSocketPath = address ? getAbstractSocketPath(address) : null;
      if (abstractSocketPath) {
        stream = net.createConnection({ path: abstractSocketPath });
        connection = createConnection({ stream });
      } else {
        connection = address ? createConnection({ busAddress: address }) : createConnection();
      }
    } catch (error) {
      stream?.destroy();
      this._handleConnectionFailure(error);
